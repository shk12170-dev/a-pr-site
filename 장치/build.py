#!/usr/bin/env python3
"""리추얼 기록·과제 목록·출석 파일을 읽어 두 사이트를 다시 만든다.
...
def 상대트랙_html(트랙메타, 배포) -> str:
    이름 = html.escape(트랙메타["상대트랙_이름"])
    반대편 = "cloud" if 트랙메타["track_id"] == "backend" else "backend"
    url = 배포.get(반대편, "").strip()
    if not url:
        return f'<span style="color:var(--muted)">{이름} 준비 중</span>'
    return f'<a href="{html.escape(url)}">{이름} 보기 →</a>'


def 제출물만들기(배포, 숫자, 과제자료):
    """짧은 확인 방법과 제출문을 만든다(BRA-C11, C12, C20)."""
    폴더 = 사업장 / "제출물"
    폴더.mkdir(parents=True, exist_ok=True)
    제출트랙 = 배포.get("제출할_트랙", "backend")
    이름표 = {"backend": "백엔드 개발자", "cloud": "클라우드 엔지니어"}
    꼬리표 = {"backend": "백엔드", "cloud": "클라우드"}
    미정 = "(배포 후 본문/배포주소.json에 적으면 채워집니다)"
    제출URL = 배포.get(제출트랙, "").strip() or 미정
    반대편 = "cloud" if 제출트랙 == "backend" else "backend"
    보조URL = 배포.get(반대편, "").strip() or 미정
    소스 = 배포.get("소스저장소", "").strip() or 미정

    확인 = [
        "# 짧은 확인 방법",
        "",
        f"- **어디로 가나요**: {제출URL}",
        f"  - {이름표[제출트랙]} 지원용 사이트입니다. 계정 만들기·로그인·비밀번호 없이 새 시크릿 창에서 바로 열립니다.",
        f"  - {이름표[반대편]} 지원용 사이트는 첫 화면 맨 위 링크로 이어집니다: {보조URL}",
        "",
        "- **세 단계 안에 무엇을 하나요**",
        ...
        "| 폴더 | 내용 |",
        "|---|---|",
        "| `문서/` | 이력서·자기소개서·경력기술서를 백엔드용·클라우드용으로 각각 3개씩, 모두 6개 |",
        "| `장치/` | 실행 파일 `build.py`, 돌리는 방법을 적은 `README.md`, 입력 파일, 마지막 실행 결과(`마지막결과/`) |",
        "| `본문/` | 장치가 읽는 자기소개 본문과 트랙별 설정. 장치를 돌리려면 이 폴더가 함께 있어야 합니다 |",
        "",
    ...
    글 = [
        ...
        f"- 함께 보는 {이름표[반대편]} 버전(첫 화면 링크로 연결): {보조URL}",
        f"- 소스: {소스}",
        "",
        "## 제출 파일",
        "",
        f"- `제출_문서와장치.zip` — 문서 6개(이력서·자기소개서·경력기술서 × 2벌) + 장치(실행 소스·README·마지막 결과) + 본문",
        "",
        "## AI와 나의 판단",
        "",
    ]
    for 번호, 키 in enumerate(
        ["AI에게_맡긴_일", "내가_직접_판단한_일", "AI_제안을_따르지_않은_일"], start=1
    ):
        제목 = {"AI에게_맡긴_일": "AI에게 맡긴 일", "내가_직접_판단한_일": "내가 직접 판단한 일", "AI_제안을_따르지_않은_일": "AI 제안을 따르지 않은 일"}[키]
        글.append(f"{번호}. **{제목}**")
        for 항목 in 판단[키]:
            글.append(f"   - {항목}")
        글.append("")
    완료 = [t for t in 과제자료["과제"] if t.get("상태") != "예정"]
    글 += [
        "## 완주 확인",
        "",
        f"- 소설에서 뽑은 자기소개 본문이 사이트 가운데에 있고, 각색된 대화·묘사는 넣지 않았습니다.",
        f"- 첫 화면에 이름과 “…한 사람”으로 끝나는 한 줄 소개가 있고, 그 안에 날짜가 있습니다.",
        f"- 숫자 칸 {len(숫자)}개에 모두 출처를 적었습니다.",
        f"- 대표작 자리에 10번 논문 파일을 직접 올렸고, 13번 앱 자리를 비워 두었습니다.",
        f"- 과제 {len(완료)}개를 이력서·경력기술서에 능력별로 나눠 적었습니다.",
        f"- 장치를 두 번 돌리고 새 폴더에서도 돌려 결과가 같은 것을 확인했습니다.",
        f"- 제출물에 본인 이름과 공개하기로 한 연락 수단 외의 개인정보, 비밀번호·토큰·API 키는 없습니다.",
        "",
    ]
...
def 문서만들기(메타, 과제자료, 숫자):
    """이력서·자기소개서·경력기술서를 트랙별로 만든다."""
    기본 = 읽기(본문폴더 / "이력_기본정보.json")
    이름 = 메타["이름"]
    목표 = 메타["목표"]
    문서폴더 = 사업장 / "문서"
    문서폴더.mkdir(parents=True, exist_ok=True)
    꼬리 = {"backend": "백엔드", "cloud": "클라우드"}[메타["track_id"]]
...
def 만들기():
    ...
    배포 = 읽기(본문폴더 / "배포주소.json")
    리추얼 = 리추얼세기()
    숫자 = 숫자모으기(리추얼, 과제자료)
    템플릿 = 템플릿파일.read_text(encoding="utf-8")
    이야기 = 본문_html()

    if 리추얼 is None:
        print("[안내] 입력/리추얼기록.json 이 없어 리추얼 숫자는 비워 두고 만들었습니다.")

    안내문 = "이 칸의 숫자는 기록 파일에서 그대로 센 것입니다. 새 기록을 넣고 장치를 한 번 돌리면 다시 계산됩니다."
    하단 = (
    ...
    for 설정 in 트랙설정:
        ...
        값 = {
            ...
            "{{첫화면_보조문}}": html.escape(메타["첫화면_보조문"]),
            "{{상대트랙_링크}}": 상대트랙_html(메타, 배포),
            "{{이야기_본문}}": 이야기
        ...
        설정["출력"].mkdir(parents=True, exist_ok=True)
        (설정["출력"] / "index.html").write_text(문서, encoding="utf-8")
        print(f"[완료] {설정['출력'].name}/index.html")
    ...
    print(f"[완료] 마지막결과/숫자.json, 마지막결과/문단후보.md")
